  ...
  <div v-if="showNothing === true">
    <div v-for="pack in packages" :key="pack.id">
      ...
      >
        ...
        <v-card-actions v-if="name === 'DispatcherState3'">
          <v-btn
              outlined
              rounded
              text
        ...
        <v-card-actions v-else-if="name === 'DealerState3'">
          <v-btn
              outlined
              rounded
              text
              @click="updateState(pack.id,4)"
          >
            Agregar pedido
          </v-btn>
        </v-card-actions>
...
export default {
  ...
  methods: {
    retrievePackageState(){
      PackageService.getAllByState(3, this.$route.params.dispatcherId)
          .then(response => {
